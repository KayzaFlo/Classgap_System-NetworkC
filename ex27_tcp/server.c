...
int main() {
    ...
    // 2. Mise en écoute (Backlog = 5) 
    listen(sfd, 5);
    printf("[TCP-Server] En attente de connexion...\n");

    while (1) {
        // 3. Acceptation : crée une NOUVELLE socket (cfd) dédiée au client 
        cfd = accept(sfd, NULL, NULL); 
        printf("[TCP-Server] Client connecté !\n");

        // 4. Communication via read/write 
        ...
        // 5. Fermeture de la session client, mais le serveur (sfd) continue 
        close(cfd);
    }
    return 0;
}